...
export class Dashboard implements OnInit {
  ...
  calculateMonthlySales(): void {
    ...
    this.salesByMonth.sort((a, b) => {
      ...
      const monthA =
        new Date(`${a.month} 1, 2026`).getMonth();

      const monthB =
        new Date(`${b.month} 1, 2026`).getMonth();
  ...
  getBarHeight(value: number): number {
    ...
    return Math.max(
      5,
      (value / this.maxMonthlySales) * 100
    );
  }
